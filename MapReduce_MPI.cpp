...
int main(int argc, char* argv[]){
    ...
    /*************************************Reader Threads********************************/
    #pragma omp parallel private(thread_num,global_thread_num,cur_item,p,strhash_value, queue_index,cur_word,temp_word,word_pointer,final_word)
    {
        ...
        if(thread_num<NUM_READERS){
            for(int i=floor(global_thread_num*NUM_FILES/NUM_TOTAL_READERS);i<NUM_FILES && i<floor((global_thread_num+1)*NUM_FILES/NUM_TOTAL_READERS);i++){
                ...
                if(i==floor((global_thread_num+1)*NUM_FILES/NUM_TOTAL_READERS)-1 || i==NUM_FILES-1) {
                    cout<<"Reader thread: "<<global_thread_num<<endl;
                    omp_set_lock(&lock[thread_num]);
                    done[thread_num]=1;
                    omp_unset_lock(&lock[thread_num]);
                    cout<<"Done set reader thread: "<<global_thread_num<<endl;
                }
            }
            cout<<"Reader done:\t"<<"Proc "<<pid<<"\tthread "<<thread_num<<endl;
    ...
    /*************************************Mapper Threads********************************/
            ...
            //cout<<"Proc "<<pid<<"\tthread "<<thread_num<<"\tqueue"<<queue_index<<endl;
            omp_set_lock(&lock[queue_index]);
            dummy_done[queue_index]=done[queue_index];
            //cout<<"lock set Proc "<<pid<<"\tthread "<<thread_num<<endl;
            omp_unset_lock(&lock[queue_index]);
            //cout<<"lock unset Proc "<<pid<<"\tthread "<<thread_num<<endl;
            ...
            cout<<"Mapper done:\t"<<"Proc "<<pid<<"\tthread "<<thread_num<<endl;
            ...
            cout<<"Reducer queue updated:\t"<<"Proc "<<pid<<"\tthread "<<thread_num<<endl;
    ...
    for(int i=0;i<NUM_REDUCERS;i++){
        ...
        cout<<"Reducer done:\t"<<"Proc "<<pid<<"\tthread "<<thread_num<<endl;
    }
    ...
    for(int i=0;i<NUM_REDUCERS;i++){
        ...
        cout<<"Writer done:\t"<<"Proc "<<pid<<"\tthread "<<thread_num<<endl;
    }
    myFile.close();
    elapsed_time+=omp_get_wtime();
    cout<<"Processor "<<pid<<" Word Count = "<<totalWords<<endl;
